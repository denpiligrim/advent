...
class AdventBotService
{
    ...
    public function handleUpdate($update)
    {
        ...
        // Если ивент идет, но заданий на сегодня больше нет
        if (!$user->current_task_id) {
            return $this->telegram->sendMessage([
                'chat_id' => $chatId,
                'text' => "🎄 Ты уже выполнил все задания на сегодня! Отдыхай, набирайся сил, и я пришлю новое задание завтра утром. \n\n🏆 Твой счет: <b>{$user->total_score}</b> баллов.",
                'parse_mode' => 'HTML'
            ]);
        }

        // Если вдруг мы дошли сюда (например, юзер просто пишет текст, когда есть активное задание-кнопка)
        return $this->telegram->sendMessage([
            'chat_id' => $chatId,
            'text' => "🎄 Чтобы выполнить задание, нажми на одну из кнопок выше!"
        ]);
    }
    ...
    protected function sendWelcome($user)
    {
        ...
        if ($today->lt($startDate)) {
            $welcomeText .= "Наш праздничный марафон начнется <b>1 января в 8:00 МСК</b>! Заходи в первый день года, тебя будут ждать интересные задания и приятный бонус в конце. До встречи! 🎅❄️";
    ...
    protected function handleCallback($user, $data)
    {
        ...
        if ($data === 'start_daily_tasks') {
            // Убираем часики на кнопке
            $this->telegram->answerCallbackQuery([
                'callback_query_id' => $this->telegram->getWebhookUpdate()->getCallbackQuery()->get('id'),
            ]);

            // Просто запускаем выдачу следующего задания
            return $this->giveNextTask($user);
        }
    }
